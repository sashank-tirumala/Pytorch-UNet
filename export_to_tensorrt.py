...
if __name__ == "__main__":
    ...
    net = UNet(
        n_channels=conf.model.n_channels,
        n_classes=conf.model.n_classes,
        bilinear=conf.model.bilinear,
    )
    ...
    torch.onnx.export(
        net,
        dummy_input,
        Path(conf.model.path).with_suffix(".onnx"),
        verbose=True,
        opset_version=11,
    )
    ...
    ort_session = onnxruntime.InferenceSession(
        Path(conf.model.path).with_suffix(".onnx"), providers=["CUDAExecutionProvider"]
    )
