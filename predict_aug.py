...
def add_snow_img(img):
    return A.functional.add_snow(img, snow_point=0.1, brightness_coeff=2.5)


def add_fog_img(img):
    return A.functional.add_fog(img, fog_coef=0.4, alpha_coef=0.08, haze_list=())


def add_rain_img(img):
...
def add_brightness(img, brightness_coeff=1.5):
    return A.functional.adjust_brightness_torchvision(img, brightness_coeff)


def add_high_brightness(img, brightness_coeff=1.5):
    return A.functional.adjust_brightness_torchvision(img, brightness_coeff)


def add_low_brightness(img, brightness_coeff=0.5):
    return A.functional.adjust_brightness_torchvision(img, brightness_coeff)
...
def add_aug(img, aug):
    if aug == "snow":
        return add_snow_img(img)
    elif aug == "fog":
        return add_fog_img(img)
    elif aug == "rain":
        return add_rain_img(img)
    elif aug == "sun_flare":
        return add_sun_flare_img(img)
    elif aug == "high_brightness":
        return add_high_brightness(img)
    elif aug == "low_brightness":
        return add_low_brightness(img)
    elif aug == "none":
        return add_none(img)
    else:
...
def prepare_for_net(img, device):
    if img.ndim == 2:
    ...
    return img_new.to(device=device, dtype=torch.float32)
...
def predict_img(
    ...
    img = add_aug(img, aug)
    inp_img = img.copy()
    img = prepare_for_net(img, device)
    with torch.no_grad():
        output = net(img).cpu()
        if net.n_classes > 1:
